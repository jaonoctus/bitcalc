...
async function fetchRates() {
  if (process.client) {
    ...
    if (localCache) {
      ...
      if (parsedCache) {
        if (DateTime.fromISO(parsedCache.updatedAt).plus({ minutes: 5 }) > DateTime.now()) {
          apiRatesData.value = parsedCache
          updatedAt.value = DateTime.fromISO(apiRatesData.value.updatedAt).toRelative()
